/**
 * This class is based on PhoneticCode.java by William B. Brogden
 * and is a slightly modified version of the original.
 */
package com.softcorporation.suggester.language.sound;

public class SoundEncoderEnglish extends SoundEncoder
{
/**
 * 
 ...
 */
  ...
  static final int maxCodeLen = 10; // 4

  public String getSoundCode(String text)
  {
    int mtsz = 0;
    boolean hard = false;
    ...
    //
    char[] inwd = text.toUpperCase().toCharArray();
    //
    String tmpS;
      ...
      { // not dup
        switch (symb)
        {
          ...
          case 'B' :
            if (!(n > 0
              && local.charAt(n - 1) == 'M'
              && (n + 1 == wdsz || (n + 2 == wdsz && local.charAt(n + 1) == 'E'))))
            {
          ...
          case 'K' :
            ...
            break;
          case 'P' :
            if ((n + 1 < wdsz) && (local.charAt(n + 1) == 'H')) // PH -> F
            {
              code.append('F');
            }
            else
